...
import { IDeathFormData, IMarriageFormData } from "@/types";
...
import { marriage } from "@/actions/certificates/marriage";

const MarriageFormSummary = () => {
  const [showConfirmation, setShowConfirmation] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);
  const [isPending, startTransition] = useTransition();
  const { formData, onHandleBack } = useMarriageFormState();
...
export default MarriageFormSummary;

interface IDetailsProps {
  schema: z.ZodObject<any>;
  formData: IMarriageFormData;
  title: string;
}
        ...
        {Object.entries(schema.shape).map(([key, _]) => (
          <li key={key} className="flex gap-4 min-w-72">
            ...
            <p className="text-muted-foreground">{`${
              formData[key as keyof IMarriageFormData]
            }`}</p>
          </li>
        ))}
      </ul>
